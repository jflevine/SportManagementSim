import {replayFrames,leagueSensitivity,replayMoney as money} from './replay-model.js?v=20260924a';
...
function venue(record){
 ...
 <g class="funding-flow"><rect x="27" y="20" width="746" height="125" rx="10" fill="#102c44"/>
...
export function mountReplay(host,record){
 ...
 host.innerHTML=`<section class="season-replay" aria-label="Your decisions in action"><div class="replay-heading"><div><span class="panel-kicker">YOUR DECISIONS IN ACTION</span><h2>Cycle ${record.cycle}, on the ground</h2></div><button class="ghost dark" data-replay="play">Play recap</button></div><p class="replay-intro">A short replay of this recorded result. Dark features were not funded. Highlighted features show your investments.</p><div class="replay-layout"><div class="venue-wrap">${venue(record)}<p class="venue-caption">Schematic view · features and attendance reflect this cycle · no additional simulation</p></div><div class="replay-story"><p class="replay-position"></p><h3 class="replay-title" tabindex="-1"></h3><p class="replay-text"></p><dl class="replay-figures"></dl><p class="replay-status" role="status" aria-live="polite"></p></div></div><div class="replay-controls"><button class="ghost dark" data-replay="previous">Previous</button><button class="ghost dark" data-replay="next">Next moment</button><button class="ghost dark" data-replay="skip">Skip to cash result</button><span>About ${frames.length*3} seconds · pause or step through at any time</span></div><details class="replay-transcript"><summary>Read the complete recap</summary>${frames.map(f=>`<p><b>${esc(f.title)}.</b> ${esc(f.text)}</p>`).join('')}</details></section>`;
