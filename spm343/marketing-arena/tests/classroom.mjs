...
const room=await call('create',{label:'REHEARSAL · 30-minute format · 24 fictional students'});code=room.code;console.log('Created rehearsal',code);
...
await call('advance');
state=await call('state');assert.equal(state.timer.remaining,120000);checks++;
await call('advance',{},undefined,true);
await call('grade',{level:'group',target:'pulse',scores:{}},{token:students[0].token},true);
await call('initial',{position:{audience:'players',objective:'awareness',text:'Target players seeking competition. Useful coaching fits the mandate and gives them a reason to try us.'}},{token:students[0].token});
...
delete base.audienceReason;delete base.riskReason;base.marketingType='through';base.authReason='Useful participation earns community trust. We trade broad reach for fit, risk board impatience, and report engagement to defend the choice.';
...
await call('advance');
await Promise.all(students.map(s=>call('defense',{text:'Keep the useful clinic because it matches player motivation. Change the crowded channel to protect reach. The model suggests engagement improved, but sales need real evidence.'},{token:s.token})));
await call('advance');
